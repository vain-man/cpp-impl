...
const int SIZE = 1000010;        //constant SIZE here
...
int euler[SIZE], st[SIZE], first[SIZE], level[SIZE];
int idx;
void eulerTour(Node *root, int l)
{
	...
	euler[idx] = root->data;
	level[idx] = l;
	if (first[root->data] == -1) first[root->data] = idx;
	idx++;
...
void build(int v, int start, int end)
{
	...
	int mid = (start + end) >> 1;
	//0-based indexing of left and right trees
	build(2 * v + 1, start, mid);
	build(2 * v + 2, mid + 1, end);

	if (level[st[2 * v + 1]] <= level[st[2 * v + 2]])
	{
		st[v] = st[2 * v + 1];
	}
	else
...
int query(int l, int r, int ss, int se, int v)
{
	if (ss > r || se < l) return idx;
	if (ss >= l && ss <= r) return st[v];

	int mid = (ss + se) >> 1;
	int left = query(l, r, ss, mid, 2 * v + 1);
	int right = query(l, r, mid + 1, se, 2 * v + 2);
