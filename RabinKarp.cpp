...
vll RabinKarp(string &s, string &t) {
	//Find all occurences of s in t in O(|s| + |t|) time
	int n = sz(s), m = sz(t);
	vll ans;
	...
	for (char ch : s) {
		s_hash = (s_hash + (ch - 'a' + 1) * ppower[i++]) % mod;
	}

	vll dp(m + 1);
	rep(i, 0, m) {
		dp[i + 1] = (dp[i] + (t[i] - 'a' + 1) * ppower[i]) % mod;
	}

	for (int i = 0; i + n - 1 < m; i++) {
		ll curr = (dp[i + n] - dp[i] + mod) % mod;
		if (curr == (s_hash * ppower[i]) % mod) {
			ans.pb(i);
		}
	}
